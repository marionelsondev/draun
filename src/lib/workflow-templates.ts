export interface WorkflowTemplate {
  /** Workflow name — used for the skill folder (draun-<name>). */
  name: string;
  description: string;
  /** Markdown body instructing the agent to drive the workflow via `draun ... --json`. */
  body: string;
}
...
export const WORKFLOW_TEMPLATES: WorkflowTemplate[] = [
  {
    name: 'spec',
    description: 'Write a SPEC.md for a new feature using the Draun SDD workflow',
...
1. If the spec folder does not exist yet, run \`draun new "<spec-name>" --json\`
   with the derived name to scaffold it. The output includes the spec slug and
   folder path.
2. Run \`draun instructions spec --json\`. The payload contains the output path
   (\`relOutputPath\`) and the expected markdown \`template\`.
3. Ask the user clarifying questions about scope and behavior until the
   requirements are unambiguous.
4. Write SPEC.md at the returned path, following the returned template.
   Describe components and behaviors — not implementation.
5. Run \`draun validate <spec-slug> --json\` and fix any reported problems.
6. Tell the user the spec is ready and that the next step is the
   \`draun-break\` skill to break it into issues — optionally preceded
   by the \`draun-analyze\` skill to review the spec's quality first.`,
  },
  {
    name: 'analyze',
    description:
...
1. Run \`draun instructions analyze --spec <spec-slug> --json\`. The payload
   contains the spec's path (\`relOutputPath\`) and the report \`template\`.
2. Run \`draun validate <spec-slug> --json\` and carry any reported problems
   into the findings.
3. Read the SPEC.md and analyze it, looking for:
...
5. Tell the user they can fix the spec (with your help or via the
   \`draun-spec\` skill), or proceed to the \`draun-break\` skill if the findings
   are acceptable.`,
  },
  {
    name: 'break',
...
1. Run \`draun instructions break --spec <spec-slug> --json\`. The payload
   contains the issue \`template\` and the spec content.
...
3. Write one markdown file per issue under \`issues/\` and an \`issues/INDEX.md\`
   ...
   returned template exactly.
4. Run \`draun validate <spec-slug> --json\` and fix any reported problems.
5. Tell the user the breakdown is ready and that the next step is the
   \`draun-implement\` skill to start implementing.`,
  },
  {
    name: 'implement',
    description:
...
\`draun status\` reflects reality: \`draun start <spec-slug> <NN> --json\` when
work on the issue begins (marks it \`[~]\` in progress), and
\`draun done <spec-slug> <NN> --json\` only when it is implemented, tested,
and reviewed (marks it \`[x]\`).

## manual mode

1. Run \`draun issues <spec-slug> --ready --json\` to list issues whose
...
the orchestrator only runs \`draun\` commands and passes plans and summaries
between subagents, never reading the code itself.

1. Run \`draun issues <spec-slug> --ready --json\` and pick the lowest-number
   ready issue.
2. Run \`draun start <spec-slug> <issue-number> --json\`.
...
4. Run \`draun done <spec-slug> <issue-number> --json\`, re-run
   \`draun issues <spec-slug> --ready --json\`, and continue with the next
   ready issue until none remain.
5. Report a final summary of every issue implemented and the test-suite
   result, and suggest the \`draun-archive\` skill.
...
1. Read the full graph with \`draun issues <spec-slug> --json\`. Schedule by
...
4. ONLY the orchestrator writes to INDEX.md, always one call at a time
   ...
   \`draun start <spec-slug> <NN> --json\` immediately before launching an
   issue's chain, and \`draun done <spec-slug> <NN> --json\` as soon as that
...
6. Verify independently with \`draun status <spec-slug> --json\` and the full
...
1. Run \`draun issues <spec-slug> --json\` and confirm every issue is done. If
   any are open, report them and stop.
2. Run \`draun validate <spec-slug> --json\` and fix any reported problems.
3. Run \`draun archive <spec-slug> --json\` to move the spec to the archive.
